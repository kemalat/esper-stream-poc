...
public class SystemInfoUtilities {
  ...
  static OperatingSystemMXBean getOperatingSystemBean() {
   ...
   */
  static long getMaxHeapMemorySize() {
   ...
   */
  static JVMArch getJVMArchitecture() {
  ...
  /**
   * Returns total physical memory in MB. If it is run in a 32-bit JVM it may return only a maximum of 4GB even if more memory is
   * available.
   *
   * @return the amount of physical memory in MB
   * @throws IOException if something goes wrong
   */
  static Long getTotalPhysicalMemorySize() throws IOException {
    ...
    try {
      memory = ((com.sun.management.OperatingSystemMXBean) operatingSystemBean)
  ...
  /**
   * Returns free physical memory in MB. If it is run in a 32-bit JVM it may return only a maximum of 2GB even if more memory is available.
   * <p>
   * <strong>Attention:</strong> The returned value is dependant on the OS. For example on Linux
   * you might see less memory than there is actually available due to the OS using it for disk caching. That memory is not actually blocked
   * because the OS would release it if requested, however this method will return the free memory which does NOT contain the ram used for
   * disk caching. Use with care!
   * </p>
   *
  ...
  private static String executeMemoryInfoProcess(String... command) throws IOException {
    ...
    throw new IOException(
        "Could not read memory process output for command " + Arrays.toString(command));
  }
   ...
   */
  static void logEnvironmentInfos() {
    ...
    try {
      if (LOGGER.isLoggable(Level.INFO)) {
        LOGGER.log(Level.INFO,
            "Maxmimum physical memory available for JVM: " + getTotalPhysicalMemorySize() + "mb");
      }
    } catch (IOException e) {
    ...
    if (LOGGER.isLoggable(Level.INFO)) {
      LOGGER.log(Level.INFO,
          "Free physical memory available for JVM: " + getFreePhysicalMemorySize() + "mb");
    }
  }
